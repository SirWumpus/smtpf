#
# Copyright 2008 by SnertSoft. All rights reserved.
#
...
name "${PRODUCT_NAME} 2.2.2"
outFile "${PRODUCT_NAME}-2.2.2-Setup.exe"
...
!define MUI_LANGDLL_REGISTRY_KEY "Software\SnertSoft\smtpf"
...
section
	System::Call 'kernel32::CreateMutexA(i 0, i 0, t "${PRODUCT_NAME}-2.2.2-Setup.exe") i .r1 ?e'
	Pop $R0
	StrCmp $R0 0 +3
	...
	nsExec::Exec 'net stop smtpf'
	Pop $status
	IntCmpU $status 0 0 +2 +2
		DetailPrint "Stopped smtpf service"

	File smtpf.exe
	nsExec::Exec '"$INSTDIR\smtpf.exe" +service'
	Pop $0
	...
	nsExec::Exec '"$INSTDIR\smtpf.exe" help="$INSTDIR/smtpf.cf" file="$INSTDIR/smtpf.cf"'
	Pop $0

	IntCmpU $status 0 0 +6 +6
		DetailPrint "Restarting smtpf service"
		; net start exit code: 0 successful start, 2 already running
		nsExec::Exec 'net start smtpf'
		Pop $status
		IntCmpU $status 0 +2
			DetailPrint "FAILED to restart smtpf service, exit=$status"
...
#	File examples\access-defaults.cf
	...
	File examples\spamassassin\smf.cf
	File examples\spamassassin\smf.pm
sectionEnd
...
get_lickey:
		...
		Goto done
check_lickey:
	nsExec::Exec '"$INSTDIR\smtpf.exe" +test-lickey lickey-file="$INSTDIR/lickey.txt"'
	Pop $0
...
section "Uninstall"
	ExecWait 'net stop smtpf'
	Sleep 2000
	ExecWait '"$INSTDIR\smtpf.exe" -service'
	Delete $INSTDIR\*.exe
	Delete $INSTDIR\*.sq3
	Delete $INSTDIR\*.log
	Rmdir /r $INSTDIR\doc
