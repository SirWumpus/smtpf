...
Stats stat_ns_bl_uri		= { STATS_TABLE_MSG,	"ns-bl-uri" };
...
uriblHeaders(Session *sess, va_list args)
{
	...
	/* Use a separate Mime object to handle parsing of select headers.
	 * This could have also been done with a custom mime_header handler
	 * if we wanted to use the existing ctx->mime object.
	 */
	if ((mime = uriMimeCreate(1)) == NULL)
		goto error1;
	...
	for (i = 0; i < VectorLength(headers); i++) {
		...
		for (table = (char **) VectorBase(search); *table != NULL; table++) {
			if (0 < (length = TextInsensitiveStartsWith(hdr, *table)) && hdr[length] == ':') {
				...
				if ((rc = uriCheckString(sess, mime, hdr + length + 1)) != SMTPF_CONTINUE)
					goto done;
			}
		}
	}
