...
#else
int
mxPrune(Vector mxlist)
{
	DnsEntry *mx;
	int i, preference = 65535;
	...
	for (i = 0; i < VectorLength(mxlist); i++) {
		...
		if (TextInsensitiveCompare(mx->address_string, optInterfaceIp.string) == 0)
...
mxConnect(Session *sess, const char *domain)
{
	...
	sess->smtp_code = SMTP_TRY_AGAIN_LATER;
#endif
	snprintf(sess->reply, sizeof (sess->reply), "451 4.4.4 no answer from %s MX" ID_MSG(496), domain, ID_ARG(sess));
/*{REPLY
While attempting to perfom a call-back, there was
no answer from any of the MX servers tried.
See <a href="summary.html#opt_call_back">call-back</a> option.
}*/
	switch (DnsGet2(DNS_TYPE_MX, 1, domain, &mxlist, &error)) {
	case DNS_RCODE_OK:
		break;
	...
	case DNS_RCODE_SERVER:
		syslog(LOG_ERR, LOG_MSG(497) "%s %s", LOG_ARGS(sess), domain, error);
/*{LOG
While attempting to perfom a call-back, there was an error
looking up the MX records of the sender's domain.
See <a href="summary.html#opt_call_back">call-back</a> option.
}*/
		snprintf(sess->reply, sizeof (sess->reply), "451 4.4.3 %s %s" ID_MSG(498) CRLF, domain, error, ID_ARG(sess));
/*{NEXT}*/
#ifdef OLD_SMTP_ERROR_CODES
		sess->smtp_error = SMTP_ERROR_TEMPORARY;
		sess->smtp_code = 451;
	...
	default:
		snprintf(sess->reply, sizeof (sess->reply), "553 5.4.4 %s %s" ID_MSG(499) CRLF, domain, error, ID_ARG(sess));
/*{REPLY
While attempting to perfom a call-back, there was an error
looking up the MX records of the sender's domain.
See <a href="summary.html#opt_call_back">call-back</a> option.
	...
	if (VectorLength(mxlist) <= 0) {
		...
		|| verb_sav.option.value
			...
			syslog(LOG_DEBUG, LOG_MSG(500) "%s has no acceptable MX", LOG_ARGS(sess), domain);
		snprintf(sess->reply, sizeof (sess->reply), "550 5.4.4 no acceptable MX for %s" ID_MSG(501) CRLF, domain, ID_ARG(sess));
/*{REPLY
While attempting to perfom a call-back,
the MX list gathered from DNS was pruned to remove hosts that resolve to localhost,
RFC 3330 reserved IP addresses that cannot be reached from the Internet, or
...
See <a href="summary.html#opt_call_back">call-back</a> option.
	...
	for (i = 0; i < VectorLength(mxlist); i++) {
		...
		|| verb_sav.option.value
			...
			syslog(LOG_DEBUG, LOG_MSG(502) "%s trying MX %d %s...", LOG_ARGS(sess), domain, mx->preference, (char *) mx->value);

		if (socketOpenClient(mx->value, SMTP_PORT, optSmtpConnectTimeout.value, NULL, &socket) == 0) {
			if (verb_smtp.option.value
#ifdef FILTER_SAV
			|| verb_sav.option.value
#endif
			)
				syslog(LOG_DEBUG, LOG_MSG(503) "%s connected to MX %d %s", LOG_ARGS(sess), domain, mx->preference, (char *) mx->value);
			*sess->reply = '\0';
			break;
		}
	}
